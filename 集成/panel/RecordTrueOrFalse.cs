...
namespace 集成.panel
{
    public partial class RecordTrueOrFalse : Form
    {
        public List<string> imgFilePath;
        public RecordTrueOrFalse()
        {
            InitializeComponent();
        }
        ...
        private void choose_Pic_Click(object sender, EventArgs e)
        {
            ...
            show_pic.BackColor = Color.Black;
            Image aPicture = PictureMerge.PictureBoxShow(img);
            Bitmap c = new Bitmap(aPicture);
            Image b = PictureMerge.GetThumbnail(c, 640, 0);
            this.show_pic.Image = b;
            skinLabel2.Visible = true;
            btn_isOk.Visible = true;
        }

        private void btn_isOk_Click(object sender, EventArgs e)
        {
            int[] RBValue = new int[30];
            string tem = "";


            foreach (var groupBoxTem in skinPanel1.Controls.OfType<GroupBox>())
            {
